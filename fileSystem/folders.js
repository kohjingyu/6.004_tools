...
var Folders = (function(){
    ...
    function textInput(submitTextAction, parent, validateFunction, defaultText, width, restore, title){
        title = title || 'Warning';
        validateFunction = validateFunction || function(text){return text.length > 0;};
        defaultText = defaultText || 'type here';

        var valid = false, canceled = true;
        var shown = false;
        width = width || parseInt(parent.width()-60);
    ...
    function renameFile(path, file_li){
        ...
        var handleRename = function(fileName, actions) {
            ...
            var validName = validFileRename(fileName, actions);
            if(validName){
    ...
    function newFSObject(parent, validateName, action, title, defaultText){
        var handleCreate = function(fileName, actions) {
            var validName = validateName(fileName, actions);
            if(validName){
                action(validName, actions);
            } else {
    ...
    function newFile(file_path, parent) {
		...
		// create buffer without leading slash!
		if (data.name[0] == '/') data.name = data.name.slice(1);
                displayFile(data);
                actions.destroy();
                refresh();
                return true;    
